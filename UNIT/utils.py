...
from data import ImageFilelist, ImageFolder, templatedatasetA_bis, templatedatasetB_bis, templatedataset, templatedataset_test, templatedatasetA, templatedatasetB, templatedatasetnew, templatedatasetnew_test
import torch
import os
import math
...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=True, data = 'A'):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.5),
                                           (0.5))]
    transform_list = [transforms.RandomCrop((height, width))] + transform_list if crop else transform_list
    ...
    transform = transforms.Compose(transform_list)
    if data=='A':
      dataset = templatedatasetnew(input_folder,transform=transform)
...
def get_data_loader_folder_test(input_folder, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=True, data = 'A'):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.5),
                                           (0.5))]
    ...
    transform = transforms.Compose(transform_list)
    if data=='A':
      dataset = templatedatasetnew_test(input_folder,transform=transform)
    else:
      dataset = templatedatasetnew_test(input_folder,transform=transform)
    loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=train, drop_last=True, num_workers=num_workers)
    return loader
