...
class UNIT_Trainer(nn.Module):
    ...
    def sample(self, x_a, x_b):
        self.eval()
        x_a_recon, x_b_recon, x_ba, x_ab, xxx_a, xxx_b = [], [], [], [], [], []
        x_a,x_b = torch.transpose(x_a, 0, -1),torch.transpose(x_b, 0, -1)
        for i in range(x_a.size(-1)):
            xx_a, xx_b = x_a[...,i].unsqueeze(0).unsqueeze(0),x_b[...,i].unsqueeze(0).unsqueeze(0)
            h_a, _ = self.gen_a.encode(xx_a)
            h_b, _ = self.gen_b.encode(xx_b)
        ...
        x_ba = torch.cat(x_ba)
        x_ab = torch.cat(x_ab)
        xxx_a, xxx_b = torch.cat(xxx_a), torch.cat(xxx_b)
        self.train()
        return xxx_a, x_a_recon, x_ab, xxx_b, x_b_recon, x_ba
