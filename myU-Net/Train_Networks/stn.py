...
from utils.losses import L1, L2, ce, soft_dice_loss, dice_loss, dice_loss_val, soft_dice_loss_old, soft_dice_loss_batch
...
def train_stnpose(learning, path, start_epoch, num_epochs, best_dice, best_dicestn, val_step, early_stopping, nets, optimizers, init_lr,channel_dim, train_loader, val_loader, patch_size, data_results, massimo, minimo, supervision, device,  asnn, mode=None, log=False):
    net1 = nets[0]
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i, (data0, data1) in enumerate(zip(train_loader0, train_loader1)):
                ...
                image0, _, pose0 = data0
                image1, _, pose1 = data1
                image = torch.cat((image0, image1), 0)
                pose = torch.cat((pose0, pose1), 0)
                rand = torch.randperm(image.size()[0])
                image = image[rand]
                pose = pose[rand]
                image = image.type(th.float).to(device)
                pose = pose.type(th.float).to(device)
        ...
        for j, (data0, data1) in enumerate(zip(val_loader0, val_loader1)):
            ...
            #pose = torch.where(pose >= (val_image[0].min()), torch.where(pose < (val_image[0].min()), pose, white), black)
            with torch.no_grad():
        ...
        print('VALIDATION (BEST AT 1) -->Average of batches of ice loss: %.4f '
              % (mdicestn))
        if mdicestn > best_dicestn:
            best_dicestn = mdicestn
            torch.save(net1.state_dict(), data_results + '/net_1.pth')
        ...
        if ((val_step * 25) == epoch) or (epoch == (num_epochs - 1)):
            ...
            for q in range(nrows):
                img = np_to_img(val_image[q, 0, :, :].data.cpu().numpy(), 'image',massimo, minimo)
                img_pose = np_to_img(pose[q, 0, :, :].data.cpu().numpy(), 'image')
                img_affine = np_to_img(x_affine[q, 0, :, :].data.cpu().numpy(), 'image',massimo, minimo)
                img_mask = np_to_img(x_affine[q, 1, :, :].data.cpu().numpy(), 'image')
                ref_ = np_to_img(reference[0,0,:,:].data.cpu().numpy(), 'image')
                # 0.0.11 Store results
                if nrows == 1:
                    axes[0].set_title("Original Test Image")
                    axes[0].imshow(img, cmap='gray', vmin=0, vmax=255.)
                    axes[1].set_title("Original Test Pose")
                    axes[1].imshow(img_pose, cmap='gray', vmin=0, vmax=255.)
                    axes[2].set_title("Similarity Test Image")
                    axes[2].imshow(img_affine, cmap='gray', vmin=0, vmax=255.)
                    axes[3].set_title("Similarity Test Pose")
                    axes[3].imshow(img_mask, cmap='gray', vmin=0, vmax=255.)
                    axes[4].set_title("Reference Pose")
                    axes[4].imshow(ref_, cmap='gray', vmin=0, vmax=255.)
                else:
                    axes[q,0].set_title("Original Test Image")
                    axes[q,0].imshow(img, cmap='gray', vmin=0, vmax=255.)
                    axes[q,1].set_title("Original Test Pose")
                    axes[q,1].imshow(img_pose, cmap='gray', vmin=0, vmax=255.)
                    axes[q,2].set_title("Similarity Test Image")
                    axes[q,2].imshow(img_affine, cmap='gray', vmin=0, vmax=255.)
                    axes[q,3].set_title("Similarity Test Pose")
                    axes[q,3].imshow(img_mask, cmap='gray', vmin=0, vmax=255.)
                    axes[q,4].set_title("Reference Pose")
                    axes[q,4].imshow(ref_, cmap='gray', vmin=0, vmax=255.)
...
def train_stnpose3d(learning, path, start_epoch, num_epochs, best_dice, best_dicestn, val_step, early_stopping, nets, optimizers, init_lr,channel_dim, train_loader, val_loader, patch_size, data_results, massimo, minimo, supervision, device,  asnn, mode=None, log=False):
    net1 = nets[0]
    optimizer1 = optimizers[0]
    num_p = sum(p.numel() for p in net1.parameters() if p.requires_grad)
    print('Number of model parameters stn:', num_p)
    ...
    ref_image = resize(ref_img2, (patch_size), order=1, mode='constant', cval=ref_img2.min(), anti_aliasing=False)
    ref_ = keep_largest(ref_image,mode = '3D')
    ref = keep_largest_mask(ref_,mode = '3D')
    ref_2 = keep_largest(ref_img2,mode = '3D')
    ref2 = keep_largest_mask(ref_2,mode = '3D')

    del ref_img, ref_img2,ref_image, ref_,ref_2
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i, (data0, data1) in enumerate(zip(train_loader0, train_loader1)):
                ...
                reference = th.as_tensor(ref, dtype=th.float).unsqueeze(0).repeat(image.size()[0], 1, 1, 1,1).to(
                        "cuda").to(device)
                ...
                if i==0:
                    ...
                    for axis in axes.flatten():
                        axis.set_axis_off()
                        axis.set_aspect('equal')
                    img0 = np_to_img(image[0, 0, 64, :, :].data.cpu().numpy(), 'image')
                    img_pose0 = np_to_img(pose[0, 0, 64, :, :].data.cpu().numpy(), 'image')
                    img_affine0 = np_to_img(x_affine[0, 0, 64, :, :].data.cpu().numpy(), 'image')
                    img_mask0 = np_to_img(x_affine[0, 1, 64, :, :].data.cpu().numpy(), 'image')
                    ref_0 = np_to_img(reference[0, 0, 64, :, :].data.cpu().numpy(), 'image')

                    img1 = np_to_img(image[0, 0, :, 64, :].data.cpu().numpy(), 'image')
                    img_pose1 = np_to_img(pose[0, 0, :, 64, :].data.cpu().numpy(), 'image')
                    img_affine1 = np_to_img(x_affine[0, 0, :, 64, :].data.cpu().numpy(), 'image')
                    img_mask1 = np_to_img(x_affine[0, 1, :, 64, :].data.cpu().numpy(), 'image')
                    ref_1 = np_to_img(reference[0, 0, :, 64, :].data.cpu().numpy(), 'image')

                    img2 = np_to_img(image[0, 0, :, :, 64].data.cpu().numpy(), 'image')
                    img_pose2 = np_to_img(pose[0, 0, :, :, 64].data.cpu().numpy(), 'image')
                    img_affine2 = np_to_img(x_affine[0, 0, :, :, 64].data.cpu().numpy(), 'image')
                    img_mask2 = np_to_img(x_affine[0, 1, :, :, 64].data.cpu().numpy(), 'image')
                    ref_2 = np_to_img(reference[0, 0, :, :, 64].data.cpu().numpy(), 'image')
                    # 0.0.11 Store results
                    axes[0, 0].set_title("Original Test Image")
                    axes[0, 0].imshow(img0, cmap='gray', vmin=0, vmax=255.)
                    axes[0, 1].set_title("Original Test Pose")
                    axes[0, 1].imshow(img_pose0, cmap='gray', vmin=0, vmax=255.)
                    axes[0, 2].set_title("Similarity Test Image")
                    axes[0, 2].imshow(img_affine0, cmap='gray', vmin=0, vmax=255.)
                    axes[0, 3].set_title("Similarity Test Pose")
                    axes[0, 3].imshow(img_mask0, cmap='gray', vmin=0, vmax=255.)
                    axes[0, 4].set_title("Reference Pose")
        ...
        for j, (data0, data1) in enumerate(zip(val_loader0, val_loader1)):
            ...
            reference = th.as_tensor(ref, dtype=th.float).unsqueeze(0).repeat(val_image.size()[0], 1, 1, 1,1).to("cuda").to(device)
            #white = torch.ones((val_image.size()[0], 1, patch_size[0], patch_size[1]), requires_grad=True).to(device)
            #black = torch.zeros((val_image.size()[0], 1, patch_size[0], patch_size[1]), requires_grad=True).to(device)
            #pose = val_image.clone()
            #pose = torch.where(pose >= (val_image[0].min()), torch.where(pose < (val_image[0].min()), pose, white), black)
            with torch.no_grad():
                x_affine, theta = net1(torch.cat((val_image,pose), dim=1))
            #grid = F.affine_grid(theta,[val_image.size()[0],val_image.size()[1],512,512,512], align_corners=False)
            #x_affine = F.grid_sample(val_image, grid, align_corners=False, mode='bilinear', padding_mode='border')
            #print(x_affine.size())
        ...
        #pose_x = keep_largest_mask_torch(x_affine)
        ncols = 5  # number of columns in final grid of images
        nrows = 3 # looking at all images takes some time
        f, axes = plt.subplots(nrows, ncols, figsize=(20, 20 * nrows / ncols))
        for axis in axes.flatten():
