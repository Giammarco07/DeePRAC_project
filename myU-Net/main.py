...
import numpy as np
import torchvision
from Networks import UNet2D, UNet3D,STN_CROP, STN, VNet3D, STN3D, ResNet3D, Modified_VGG16, PNet3D, REDCNN, RetinaNet
...
if input =='children' and (network=='nnunet3D' or network=='pnet3D' or network=='p-nnunet3D'):
    if mode=='train':
...
if mode == 'train':
    ...
    if num_images_per_epoch>=500 and asnnunet==True:
        if (network == 'nnunet3D' or network == 'pnet3D'):
...
if args.continue_training and mode=='train':
    ...
    start_epoch = 0
    val_step=0
    if network=='nnunet3D' or network == 'pnet3D' or network=='redcnn':
        if dda:
            early_stopping = num_epochs
        else:
            early_stopping = int(num_epochs / 2)
...
if mode=='train':
    optimizers = []
    if network == 'nnunet3D' or network == 'nnunet2D' or network=='nnunet2.5D' or network=='redcnn':
