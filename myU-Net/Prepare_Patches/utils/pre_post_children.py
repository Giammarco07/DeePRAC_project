...
def preprocessing_c(img_path, label_path, patch_size, channel_dim, in_c, mode, res, preprocessing, do_seg=True, rsz=False, input=None):
    ...
    if mode==2:
        ...
        doverlap_stepsize = int(round(dpatch_size / 2)) #4
        hoverlap_stepsize = int(round(hpatch_size / 2)) #4
        woverlap_stepsize = int(round(wpatch_size / 2)) #4
    ...
    if in_c ==2:
        ...
        img_rrr = np.rollaxis(img_rr, 2, 1)
        print('adjusting dimension: ', img_rrr.shape)
        x_1 = adjust_dim(img_rrr, img_rrr.shape[0] + dpatch_size, img_rrr.shape[1] + hpatch_size,
                        img_rrr.shape[2] + wpatch_size)
