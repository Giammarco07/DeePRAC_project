...
class net(nn.Module):
    ...
    def forward(self, input):
        # for now it only supports one GPU
            ...
            decoder14 = self.deconvblock13(skip14)
            output1 = self.outputblock1(decoder14)
            ...
            decoder24 = self.deconvblock23(skip24)
            output2 = self.outputblock2(decoder24)
            ...
            skip33 = torch.cat([decoder32, encoder31], 1)
            decoder33 = self.deconvblock33(skip33)
            skip34 = torch.cat([decoder33, encoder11], 1)
            decoder34 = self.deconvblock33(skip34)
            output3 = self.outputblock3(decoder34)
